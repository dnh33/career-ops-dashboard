...
async def tracker_list(
    ...
    start = (page - 1) * limit
    return {
        "entries": [e.model_dump() for e in entries[start : start + limit]],
        "total": total,
        "page": page,
        "limit": limit,
    }
...
@router.patch("/tracker", tags=["tracker"])
async def tracker_patch(patch: TrackerPatch) -> dict:
    """Update an existing tracker entry by number."""
    ...
    for i, e in enumerate(entries):
        if e.number == patch.number:
            if patch.status is not None:
                entries[i].status = patch.status
            if patch.applied is not None:
                entries[i].applied = patch.applied
            if patch.company is not None:
                entries[i].company = patch.company
            if patch.role is not None:
                entries[i].role = patch.role
            if patch.link is not None:
                entries[i].link = patch.link
            _write_entries(entries)
            return {"ok": True, "entry": entries[i].model_dump()}

    raise HTTPException(status_code=404, detail=f"Entry #{patch.number} not found")
